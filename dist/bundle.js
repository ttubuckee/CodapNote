...
function Timer() {
    ...
    this.start = () => {
        if (!this.checkValue()) {
            this.resetInput();
            sweetalert__WEBPACK_IMPORTED_MODULE_0___default()('정상적인 값을 입력해주세요!');
        } else {
    ...
    this.checkValue = () => {
        return input_arr.map(e=>{
            console.log(e.value);
            return e.value;
        }).reduce((acc,cur,idx)=>acc && (this.isNumber(cur.value) || cur.value === ''));
        // const a = input_arr.reduce((acc, cur, idx) => {
        //     console.log(idx,acc);
        //     return acc && (this.isNumber(cur.value) || cur.value === '')
        // });
